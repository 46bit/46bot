...
use libc::{c_int, socklen_t, sockaddr_storage, sockaddr_in, sockaddr_in6};

// Simplified from private sys IsMinusOne trait. This is much simpler.
...
impl BoundTcpStream {
    ...
    pub fn new(source_addr: SocketAddr) -> io::Result<BoundTcpStream> {
    ...
    pub fn bind(&mut self, source_addr: SocketAddr) -> io::Result<c_int> {
    ...
    /// `ToSocketAddrs` trait can be supplied for the address; see this trait
    /// documentation for concrete examples.
    pub fn connect(&mut self, dest_addr: SocketAddr) -> io::Result<TcpStream> {
