...
impl TcpSocket {
    ...
    fn sockaddr(&self, addr: SocketAddr) -> (libc::sockaddr_storage, usize) {
        match addr {
    ...
    fn sockaddr_in4(&self, addr: SocketAddrV4) -> (libc::sockaddr_storage, usize) {
        ...
        let sockaddr = unsafe { *(&mut sockaddr as *mut _ as *mut libc::sockaddr_storage) };
        let sockaddr_size = mem::size_of::<libc::sockaddr_in>();
    ...
    fn sockaddr_in6(&self, addr: SocketAddrV6) -> (libc::sockaddr_storage, usize) {
        ...
        let sockaddr = unsafe { *(&mut sockaddr as *mut _ as *mut libc::sockaddr_storage) };
        let sockaddr_size = mem::size_of::<libc::sockaddr_in6>();
        (sockaddr, sockaddr_size)
    }
}
